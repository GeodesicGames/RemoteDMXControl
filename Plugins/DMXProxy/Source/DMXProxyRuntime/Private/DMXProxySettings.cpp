...
void FDMXProxySettingsProxy::HandleListenSocketDataReceived(const TSharedPtr<FArrayReader, ESPMode::ThreadSafe>& Data, const FIPv4Endpoint& Sender)
{
	if (bIsLog)
	{
		...
		FString FormattedTimeStamp = FString::Printf(TEXT("%02i:%02i:%02i.%03i"), 
			CurrentDateAndTime.GetHour(), 
			CurrentDateAndTime.GetMinute(), 
			CurrentDateAndTime.GetSecond(), 
			CurrentDateAndTime.GetMillisecond());

		UE_LOG(DMXProxySettingsProxy, Log, TEXT("[NUM] : %d, %s"), Data->Num(), *FormattedTimeStamp);
	}
